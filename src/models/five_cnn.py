## Creates the design of a CNN model for stock price trend across five days. 

## Import necessary packages.
from keras import Sequential, layers

## Initializes the model where five days is the input.
def initialize_model(input_shape = (32, 15, 1), num_classes = 1):
    '''
    Creates and returns the design for a five day CNN model.

    Args:
        input_shape (tuple): The shape of the input image, defaulted to (32, 15, 1).
        num_classes (int): The number of classes in the output neuron. Defaulted to 1 since model uses 
                            binary classification.

    Returns:
        model (Sequential): Returns the sequential design of the five day model.
    '''
    ...
    model.add(layers.Conv2D(filters = 64, kernel_size = (5, 3), strides = (1, 1), dilation_rate = (1, 1),\
    ...
    model.add(layers.Dense(units = 15360))
    model.add(layers.LeakyReLU(alpha = 0.01))
