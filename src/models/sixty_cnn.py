## Creates the design of a CNN model for stock price trend across sixty days. 

## Import necessary packages.
from keras import Sequential, layers

## Initializes the model where sixty days is the input.
def initialize_model(input_shape = (96, 180, 1), num_classes = 1):
    '''
    Creates and returns the design for a sixty day CNN model.
    ...
    Returns:
        model (Sequential): Returns the sequential design of the sixty day model.
    '''
    ...
    ## Create the fourth 2D Convolutional layer.
    model.add(layers.Conv2D(filters = 512, kernel_size = (5, 3), padding = 'same', strides = (1, 1), dilation_rate = (1, 1)))
    model.add(layers.BatchNormalization())
    model.add(layers.LeakyReLU(alpha = 0.01))
    ## Create the fourth Max Pooling layer.
    model.add(layers.MaxPooling2D(pool_size = (2, 1)))
    ...
    model.add(layers.Dense(units = 184320))
    model.add(layers.LeakyReLU(alpha = 0.01))
